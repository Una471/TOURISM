...
PAGE_TITLE="Savannah Gates Hotel | Analytics"
ACCENT="#86593a"; ACCENT_DARK="#5b3924"; ACCENT2="#c39a63"; SOFT="#f3ede8"
OVERVIEW_URL="https://una471.github.io/portfolio/projects/hospitality/overview.html"; OVERVIEW_LABEL="Revenue Overview"; SOFTWARE_URL="https://hotel-systemm.streamlit.app/?view=software"; 
HERO_IMAGE="savannah-hero.webp"; SEARCH_TEXT="Search revenue, guests, staffing or reports..."
...
@st.cache_data
def load_data():
    b=pd.read_csv(ROOT/"daily_bookings_analyzed.csv",parse_dates=['date']); r=pd.read_csv(ROOT/"guest_reviews_analyzed.csv",parse_dates=['date']); s=pd.read_csv(ROOT/"staffing_data_analyzed.csv",parse_dates=['date']); return b,r,s
bookings,reviews,staff=load_data()
with st.sidebar:
    sidebar_brand("SG", "Savannah Gates", "Hospitality operations · Botswana")
    page=st.radio("Go to",["Revenue Overview","Guest Satisfaction","Staffing","Pricing Strategy"],index=0,label_visibility="collapsed")
    st.divider(); guest_types=["All Guest Types"]+sorted(reviews.guest_type.dropna().unique().tolist()); platforms=["All Platforms"]+sorted(reviews.platform.dropna().unique().tolist()); guest_sel=st.selectbox("Guest Type",guest_types,index=0); platform_sel=st.selectbox("Review Platform",platforms,index=0)
    st.divider(); st.caption("Period: Jan to Dec 2025"); st.markdown('<div class="side-promo"><div><strong>Better stays. Smarter decisions.</strong><span>Revenue and guest experience for a stronger hospitality business.</span></div></div>',unsafe_allow_html=True)
rdf=reviews.copy()
if guest_sel!="All Guest Types": rdf=rdf[rdf.guest_type==guest_sel]
if platform_sel!="All Platforms": rdf=rdf[rdf.platform==platform_sel]
search_query=utilitybar("Hotel Manager")
rdf=apply_search(rdf,search_query)

def money(x): return f"P{x/1e6:.2f}M" if abs(x)>=1e6 else f"P{x/1e3:.0f}K" if abs(x)>=1e3 else f"P{x:,.0f}"

def overview():
    hero("Revenue & Guest Experience","Revenue, occupancy, guest sentiment and staffing in one compact management view.")
    rev=bookings.daily_revenue_bwp.sum(); occ=bookings.occupancy_rate_pct.mean(); rate=bookings.avg_room_rate_bwp.mean(); rating=rdf.rating.mean() if len(rdf) else np.nan; reviews_n=len(rdf); over=int(staff.overstaffed.sum())
    cols=st.columns(6); vals=[("Annual Revenue",money(rev),"Full year 2025"),("Average Occupancy",f"{occ:.1f}%","Rooms occupied"),("Avg Room Rate",money(rate),"Per occupied room"),("Guest Rating",f"{rating:.2f} / 5" if pd.notna(rating) else "—",f"{reviews_n:,} reviews"),("Overstaffed Shifts",f"{over:,}","Staffing opportunities"),("Room Nights",f"{bookings.rooms_occupied.sum():,}","Occupied rooms")]
    for i,v in enumerate(vals): cols[i].markdown(kcard(*v,primary=i==0),unsafe_allow_html=True)
    c1,c2,c3=st.columns([1.3,1,1])
    with c1:
        panel_head("Monthly Revenue Trend","Trading performance across the year"); q=bookings.assign(month=bookings.date.dt.to_period('M').astype(str)).groupby('month').daily_revenue_bwp.sum().reset_index(); fig=px.line(q,x='month',y='daily_revenue_bwp',markers=True,color_discrete_sequence=[ACCENT]); st.plotly_chart(chart_style(fig,255,False),use_container_width=True)
    with c2:
        panel_head("Occupancy by Day","Average occupancy by weekday"); order=['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday']; q=bookings.groupby('day_of_week').occupancy_rate_pct.mean().reindex(order).reset_index(); fig=px.bar(q,x='day_of_week',y='occupancy_rate_pct',color_discrete_sequence=[ACCENT2]); st.plotly_chart(chart_style(fig,255,False),use_container_width=True)
    with c3:
        panel_head("Review Sentiment","Guest feedback distribution"); q=rdf.sentiment.value_counts().reset_index(); q.columns=['sentiment','count']; fig=px.pie(q,names='sentiment',values='count',hole=.58,color='sentiment',color_discrete_map={'Positive':ACCENT,'Neutral':'#c8b79e','Negative':'#b85c50'}); st.plotly_chart(chart_style(fig,255,False),use_container_width=True)
    st.markdown('<div class="section-title">Staffing Cost by Role</div>',unsafe_allow_html=True); q=staff.groupby('role').agg(annual_cost=('daily_cost_bwp','sum'),overstaffed_shifts=('overstaffed','sum'),avg_staff=('actual_staff','mean')).reset_index(); st.dataframe(q,use_container_width=True,hide_index=True,height=300)

def guest():
    hero("Guest Satisfaction","Review quality, issue patterns and platform performance.")
    c1,c2=st.columns(2)
    with c1:
        q=rdf.rating.value_counts().sort_index().reset_index(); q.columns=['rating','count']; fig=px.bar(q,x='rating',y='count',color_discrete_sequence=[ACCENT]); st.plotly_chart(chart_style(fig,310,False),use_container_width=True)
    with c2:
        q=rdf.groupby('platform').agg(reviews=('review_id','count'),rating=('rating','mean')).reset_index(); fig=px.bar(q,x='platform',y='rating',text='reviews',color_discrete_sequence=[ACCENT2]); st.plotly_chart(chart_style(fig,310,False),use_container_width=True)
    st.dataframe(rdf[['date','platform','guest_type','rating','sentiment','issues']].sort_values('date',ascending=False).head(30),use_container_width=True,hide_index=True,height=360)

def staffing_page():
    hero("Staffing","Match labor levels to room demand and highlight avoidable staffing cost.")
    q=staff.groupby('role').agg(cost=('daily_cost_bwp','sum'),overstaffed=('overstaffed','sum'),actual=('actual_staff','mean'),optimal=('optimal_staff','mean')).reset_index(); st.dataframe(q,use_container_width=True,hide_index=True,height=420)

def pricing():
    hero("Pricing Strategy","Understand rate, occupancy and event effects before changing room pricing.")
    fig=px.scatter(bookings,x='occupancy_rate_pct',y='avg_room_rate_bwp',color='event_boost',color_continuous_scale=['#d7c4ad',ACCENT]); st.plotly_chart(chart_style(fig,420,False),use_container_width=True)

{"Revenue Overview":overview,"Guest Satisfaction":guest,"Staffing":staffing_page,"Pricing Strategy":pricing}[page]()
